...
internal class ReaderSnippetGen
{
    ...
    private MemberDeclarationSyntax MakeReaderCreateMethod()
    {
        return MethodDeclaration(
                _names.ReaderStruct.IdentifierName,
                _names.ReaderCreateMethod.Identifier
            )
    ...
    private IEnumerable<MemberDeclarationSyntax> MakeReaderStructMembers()
    {
        yield return FieldDeclaration(
                VariableDeclaration(_names.Type<DeserializerState>(Nullability.NonNullable))
    ...
    private IEnumerable<MemberDeclarationSyntax> MakeGroupReaderStructMembers()
    {
        ...
        yield return ConstructorDeclaration(_names.ReaderStruct.Identifier)
            ...
            .WithParameterList(
                ParameterList(
                    SingletonSeparatedList(
                        Parameter(_names.GroupReaderContextArg.Identifier)
                            .WithType(_names.Type<DeserializerState>(Nullability.NonNullable))
                    )
                )
            )
    ...
    private PropertyDeclarationSyntax MakeReaderProperty(
        TypeSyntax type,
        string name,
    ...
    private PropertyDeclarationSyntax MakeReadProperty(
        TypeSyntax type,
        string name,
        SimpleNameSyntax readName,
        object indexOrBitOffset,
        ExpressionSyntax secondArg,
        Func<ExpressionSyntax, ExpressionSyntax> cast,
        bool cond
    )
    {
        var right = InvocationExpression(
            ...
            .AddArgumentListArguments(Argument(ValueOf(indexOrBitOffset)));
    ...
    {
        var typeSyntax = _names.MakeTypeSyntax(
            field.Type,
            field.DeclaringType,
            TypeUsage.Reader,
    ...
    private PropertyDeclarationSyntax MakeReadStructProperty(Field field)
    {
        ...
        return MakeReadProperty(
            qtype,
            _names.GetCodeIdentifier(field).ToString(),
            nameof(DeserializerState.ReadStruct),
            (int)field.Offset,
            creator,
            null,
            field.DiscValue.HasValue
    ...
    private PropertyDeclarationSyntax MakeReadGroupProperty(Field field)
    {
        ...
        var right = ObjectCreationExpression(type)
            .WithArgumentList(
                ArgumentList(
                    SingletonSeparatedList(Argument(_names.ReaderContextField.IdentifierName))
                )
            );
    ...
    private ExpressionSyntax MakeReadListPropertyImpl(
        Type elementType,
    ...
    {
        var elementTypeSyntax = _names.MakeTypeSyntax(
            elementType,
            scope,
            TypeUsage.Reader,
            Nullability.NonNullable
        ...
        if (elementType.Tag == TypeTag.Interface || elementType.Tag == TypeTag.CapabilityPointer)
        {
            if (depth == 0)
        ...
        switch (elementType.Tag)
        {
            ...
            case TypeTag.Struct:
            {
                return InvocationExpression(
                    ...
                    .AddArgumentListArguments(Argument(MakeReaderCreator(elementTypeSyntax)));
            }
    ...
    {
        var type = _names.MakeTypeSyntax(
            field.Type,
            field.DeclaringType,
            TypeUsage.Reader,
            Nullability.NonNullable
    ...
    {
        var context = InvocationExpression(
                MemberAccessExpression(
                    SyntaxKind.SimpleMemberAccessExpression,
                    _names.ReaderContextField.IdentifierName,
                    IdentifierName(nameof(DeserializerState.ReadList))
    ...
    private PropertyDeclarationSyntax MakeReaderFieldProperty(Field field)
    {
        ...
        {
            case TypeTag.Bool:
                return MakeReadPrimitiveProperty<bool>(
                    field,
                    nameof(SerializerExtensions.ReadDataBool)
                );

            case TypeTag.S8:
                return MakeReadPrimitiveProperty<sbyte>(
                    field,
                    nameof(SerializerExtensions.ReadDataSByte)
                );

            case TypeTag.U8:
                return MakeReadPrimitiveProperty<byte>(
                    field,
                    nameof(SerializerExtensions.ReadDataByte)
                );

            case TypeTag.S16:
                return MakeReadPrimitiveProperty<short>(
                    field,
                    nameof(SerializerExtensions.ReadDataShort)
                );

            case TypeTag.U16:
                return MakeReadPrimitiveProperty<ushort>(
                    field,
                    nameof(SerializerExtensions.ReadDataUShort)
                );

            case TypeTag.S32:
                return MakeReadPrimitiveProperty<int>(
                    field,
                    nameof(SerializerExtensions.ReadDataInt)
                );

            case TypeTag.U32:
                return MakeReadPrimitiveProperty<uint>(
                    field,
                    nameof(SerializerExtensions.ReadDataUInt)
                );

            case TypeTag.S64:
                return MakeReadPrimitiveProperty<long>(
                    field,
                    nameof(SerializerExtensions.ReadDataLong)
                );

            case TypeTag.U64:
                return MakeReadPrimitiveProperty<ulong>(
                    field,
                    nameof(SerializerExtensions.ReadDataULong)
                );

            case TypeTag.F32:
                return MakeReadPrimitiveProperty<float>(
                    field,
                    nameof(SerializerExtensions.ReadDataFloat)
                );

            case TypeTag.F64:
                return MakeReadPrimitiveProperty<double>(
                    field,
    ...
    public StructDeclarationSyntax MakeReaderStruct(TypeDefinition def)
    {
        ...
        var members =
            def.Tag == TypeTag.Group ? MakeGroupReaderStructMembers() : MakeReaderStructMembers();
