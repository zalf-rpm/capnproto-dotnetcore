...
internal class InterfaceSnippetGen
{
    ...
    private TypeSyntax TransformReturnType(Method method)
    {
        switch (method.Results.Count)
        {
            ...
            case 1 when method.Results[0].Type.Tag == TypeTag.Struct:
                return GenericName(nameof(Task))
                    .AddTypeArgumentListArguments(
                        _names.MakeTypeSyntax(
                            method.Results[0].Type,
        ...
        if (method.Params.Count > 0)
        {
            ...
            if (arg0.Name == null)
                ...
                foreach (var arg in method.Params)
                    list.Add(
                        Parameter(Identifier(_names.GetCodeIdentifierLowerCamel(arg)))
                            .WithType(
                                _names.MakeTypeSyntax(
                                    arg.Type,
                                    method.DeclaringInterface,
    ...
    {
        var ifaceDecl = InterfaceDeclaration(
                _names.MakeTypeName(type, NameUsage.Interface).Identifier
            )
            .AddModifiers(Public)
            .AddAttributeLists(
                _names
                    .MakeTypeDecorationAttributes(type.Id)
                    .AddAttributes(
                        Attribute(IdentifierName("Proxy"))
                            .AddArgumentListArguments(
                                AttributeArgument(
                                    TypeOfExpression(
                                        _names.MakeGenericTypeNameForAttribute(
                                            type,
                                            NameUsage.Proxy
                                        )
                                    )
        ...
        if (type.Superclasses.Count == 0)
            ifaceDecl = ifaceDecl.AddBaseListTypes(
                SimpleBaseType(IdentifierName(nameof(IDisposable)))
            );
        else
    ...
    private StatementSyntax MakeProxyReturnResult(Method method)
    {
        if (method.ResultStruct.Definition.SpecialName == SpecialName.MethodResultStruct)
        {
            ...
            return ReturnStatement(
                TupleExpression().AddArguments(MakeProxyReturnResultTupleElements(method).ToArray())
            );
        }
    ...
    private StatementSyntax MakeProxyCreateResult(Method method)
    {
        ...
        var domainType = _names.MakeTypeSyntax(
            resultType,
            method.DeclaringInterface,
            TypeUsage.DomainClass,
            Nullability.NonNullable
        ...
        ExpressionSyntax createDomain = InvocationExpression(
            ...
            .AddArgumentListArguments(Argument(_names.DeserializerLocal.IdentifierName));
        ...
        foreach (var method in allMethods)
        {
                                        ...
                                        EqualsValueClause(
                                            InvocationExpression(
                                                MemberAccessExpression(
                                                    ...
                                                    GenericName(
                                                            Identifier(
                                                                nameof(SerializerState.CreateForRpc)
                                                            )
                                                        )
                ...
                bodyStmts.Add(
                    LocalDeclarationStatement(
                        VariableDeclaration(IdentifierName("var"))
                            .WithVariables(
                                SingletonSeparatedList(
                                    VariableDeclarator(_names.AnonymousParameter.Identifier)
                                        .WithInitializer(
                                            EqualsValueClause(
                                                            ...
                                                            SeparatedList<ExpressionSyntax>(
                                                                CommonSnippetGen
                                                                    .MakeCommaSeparatedList(
                                                                        MakeProxyCallInitializerAssignments(
                                                                            method
                                                                        )
                                                                    )
                                                                    .ToArray()
            ...
            var call = InvocationExpression(IdentifierName(nameof(BareProxy.Call)))
                .AddArgumentListArguments(
                        ...
                        InvocationExpression(
                                MemberAccessExpression(
                                    ...
                                    GenericName(nameof(SerializerState.Rewrap))
                                        .AddTypeArgumentListArguments(
                                            _names.Type<DynamicSerializerState>(
                                                Nullability.NonNullable
                                            )
                                        )
                                )
                    ...
                    Argument(_names.CancellationTokenParameter.IdentifierName)
                );
            ...
            else
            {
                ...
                bodyStmts.Add(
                    UsingStatement(
                            Block(MakeProxyCreateResult(method), MakeProxyReturnResult(method))
                        )
                        .WithDeclaration(
                            VariableDeclaration(IdentifierName("var"))
                                .AddVariables(
                                    VariableDeclarator(_names.DeserializerLocal.Identifier)
                                        .WithInitializer(
                                            EqualsValueClause(AwaitExpression(whenReturned))
                                        )
                                )
                        )
                );
    ...
    private StatementSyntax MakeSkeletonMethodSerializerLocalDeclaration(Method method)
    {
        return LocalDeclarationStatement(
            VariableDeclaration(IdentifierName("var"))
                .WithVariables(
                    SingletonSeparatedList(
                        VariableDeclarator(_names.SerializerLocal.Identifier)
                            .WithInitializer(
                                EqualsValueClause(
                                    InvocationExpression(
                                        MemberAccessExpression(
    ...
    {
        var block = Block(MakeSkeletonMethodSerializerLocalDeclaration(method));
            ...
            block = block.AddStatements(
                ExpressionStatement(
                    InvocationExpression(
                        ...
                        .AddArgumentListArguments(Argument(_names.SerializerLocal.IdentifierName))
                )
            );

        block = block.AddStatements(ReturnStatement(_names.SerializerLocal.IdentifierName));
    ...
    private IEnumerable<StatementSyntax> MakeSkeletonMethodBody(Method method)
    {
        ...
        if (method.GenericParameters.Count == 0)
            ...
            methodName = GenericName(_names.GetCodeIdentifier(method).Identifier)
                .AddTypeArgumentListArguments(
                    method
                        .GenericParameters.Select(p =>
                            _names.GetGenericTypeParameter(p).IdentifierName
                        )
                        .ToArray()
                );
        ...
        if (method.Params.Count > 0)
        {
            ...
            if (method.ParamsStruct.Definition.SpecialName == SpecialName.MethodParamsStruct)
            {
                ...
                call = call.AddArgumentListArguments(MakeSkeletonMethodCallArgs(method).ToArray());
            }
            else
            {
                call = call.AddArgumentListArguments(Argument(createDomain));
            }
        }
        ...
        else
        {
            ...
            if (method.ResultStruct.Definition.SpecialName == SpecialName.MethodResultStruct)
            {
                if (method.Results.Count == 1)
                {
                    lambdaArg = SimpleLambdaExpression(
                        Parameter(
                            Identifier(_names.GetCodeIdentifierLowerCamel(method.Results.Single()))
                        ),
                ...
                else
                {
                    ...
                    lambdaArg = ParenthesizedLambdaExpression(
                        ParameterList(SeparatedList<ParameterSyntax>(paramList)),
                        MakeMaybeTailCallLambdaBody(method)
                    );
                }
            }
            ...
            var maybeTailCall = InvocationExpression(
                ...
                .AddArgumentListArguments(Argument(call), Argument(lambdaArg));
    ...
    private IEnumerable<MemberDeclarationSyntax> MakeSkeletonMethods(TypeDefinition def)
    {
        foreach (var method in def.Methods)
        {
            var methodDecl = MethodDeclaration(
                ...
                .AddBodyStatements(
                    UsingStatement(Block(MakeSkeletonMethodBody(method).ToArray()))
                        .WithExpression(_names.DeserializerLocal.IdentifierName)
                );
        ...
        var classDecl = ClassDeclaration(name)
            ...
            .AddMembers(
                ...
                ConstructorDeclaration(name)
                    ...
                    .AddBodyStatements(
                        ExpressionStatement(
                            InvocationExpression(IdentifierName(SkeletonWorder.SetMethodTableName))
                                .AddArgumentListArguments(
                ...
                // InterfaceId
                PropertyDeclaration(
                        _names.Type<ulong>(Nullability.NonNullable),
                        nameof(Skeleton<object>.InterfaceId)
                    )
                    .AddModifiers(Public, Override)
                    .WithExpressionBody(ArrowExpressionClause(ValueOf(type.Id)))
                    .WithSemicolonToken(Token(SyntaxKind.SemicolonToken))
            );
    ...
    private LocalFunctionStatementSyntax MakeLocalAwaitProxyFunction(
        Method method,
        IReadOnlyList<Field> path
    )
    {
        ...
        ExpressionSyntax memberAccess = ParenthesizedExpression(
            AwaitExpression(_names.TaskParameter.IdentifierName)
        );
        ...
        return LocalFunctionStatement(
                GenericName(Identifier(nameof(Task)))
                    .WithTypeArgumentList(TypeArgumentList(SingletonSeparatedList(idisposable))),
    ...
    public IEnumerable<MemberDeclarationSyntax> MakePipeliningSupport(TypeDefinition type)
    {
        ...
        foreach (var path in ExpandPipeliningPaths(method))
        {
            ...
            var capTypeSyntax = _names.MakeTypeSyntax(
                capType,
                null,
                TypeUsage.DomainClass,
                Nullability.NonNullable
            ...
            var pathDecl = FieldDeclaration(
                    VariableDeclaration(IdentifierName(nameof(MemberAccessPath)))
                        .AddVariables(
                            VariableDeclarator(accessPath.Identifier)
                                .WithInitializer(
                                    EqualsValueClause(
                                        ObjectCreationExpression(
            ...
            var methodDecl = MethodDeclaration(capTypeSyntax, methodName.Identifier)
                ...
                .AddParameterListParameters(
                    Parameter(_names.TaskParameter.Identifier)
                        .AddModifiers(This)
                            ...
                            InvocationExpression(
                                    MemberAccessExpression(
                                        ...
                                        GenericName(
                                            ...
                                            .AddTypeArgumentListArguments(capTypeSyntax)
                                    )
                                )
                                .AddArgumentListArguments(
                                    Argument(
                                        InvocationExpression(
                                            ...
                                            .AddArgumentListArguments(
                                                Argument(_names.TaskParameter.IdentifierName),
                                                Argument(accessPath.IdentifierName),
                                                Argument(
                                                    InvocationExpression(
