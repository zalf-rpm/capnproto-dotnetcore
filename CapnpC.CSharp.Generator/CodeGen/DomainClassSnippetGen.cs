...
internal class DomainClassSnippetGen
{
    ...
    private MemberDeclarationSyntax MakeUnionField(Field field)
    {
        ...
        switch (field.Type.Tag)
        {
            ...
            default:
                return PropertyDeclaration(type, _names.GetCodeIdentifier(field).Identifier)
                    .AddModifiers(Public)
    ...
    private MemberDeclarationSyntax MakeStructField(Field field)
    {
        ...
        if (field.DefaultValueIsExplicit && field.Type.IsValueType)
            prop = prop.WithInitializer(EqualsValueClause(MakeDefaultValue(field)))
                .WithSemicolonToken(Token(SyntaxKind.SemicolonToken));
    ...
    private IEnumerable<ExpressionSyntax> MakeInitializerAssignments(
        Value structValue,
    ...
    private ExpressionSyntax MakeListSerializeParticle(
        Type type,
        ExpressionSyntax writer,
        ExpressionSyntax domain
    )
    {
    ...
    private ExpressionSyntax MakeComplexSerializeParticle(
        Type type,
        ExpressionSyntax writer,
        ExpressionSyntax domain
    )
    ...
    private ExpressionSyntax ConditionalSuppressNullableWarning(
            ...
            case TypeTag.StructPointer:
                return ExpressionStatement(
                    InvocationExpression(
                            MemberAccessExpression(
                                SyntaxKind.SimpleMemberAccessExpression,
                                ConditionalSuppressNullableWarning(
                                    MemberAccessExpression(
                                        SyntaxKind.SimpleMemberAccessExpression,
                                        _names.WriterParameter.IdentifierName,
                                        _names.GetCodeIdentifier(field).IdentifierName
            ...
            case TypeTag.Group:
                return ExpressionStatement(
                    MakeComplexSerializeParticle(
                        field.Type,
                        field.DiscValue.HasValue
                            ? _names.SuppressNullableWarning(writerProp)
                            : writerProp,
                        _names.GetCodeIdentifier(field).IdentifierName
                    )
                );
    ...
    private StatementSyntax MakeApplyDefaultsMethodFieldAssignment(Field field)
    {
        ...
        return ExpressionStatement(
            AssignmentExpression(
                SyntaxKind.SimpleAssignmentExpression,
                lhs,
                BinaryExpression(SyntaxKind.CoalesceExpression, lhs, rhs)
            )
        );
    }

    private ExpressionSyntax MakeInnerStructListConversion(
                                        ...
                                        MemberAccessExpression(
                                            ...
                                            GenericName(nameof(CapnpSerializable.Create))
                                                .AddTypeArgumentListArguments(
                                                    MakeNonNullableType(elementType)
                                                )
                                        )
                                    )
    ...
    private ExpressionSyntax MakeStructListConversion(
        ExpressionSyntax context,
        TypeSyntax elementType,
        int rank
    )
                ...
                .AddArgumentListArguments(
                    Argument(
                        SimpleLambdaExpression(
                            Parameter(Identifier("_")),
                            CastExpression(
                                _names.Type<object>(Nullability.NonNullable),
                                IdentifierName("_")
                            )
                        )
                    )
    ...
    private IEnumerable<StatementSyntax> MakeSerializeStatements(TypeDefinition def)
    {
        ...
        var nondiscFields = def.Fields.Where(f =>
            !f.DiscValue.HasValue && f.Type.Tag != TypeTag.Void
        );
    ...
    {
        var relevantFields = def.Fields.Where(f =>
            !f.DiscValue.HasValue
    ...
    private MemberDeclarationSyntax MakeDeserializeMethod(TypeDefinition def)
    {
        ...
        stmts.Add(
            ExpressionStatement(InvocationExpression(_names.ApplyDefaultsMethod.IdentifierName))
        );
