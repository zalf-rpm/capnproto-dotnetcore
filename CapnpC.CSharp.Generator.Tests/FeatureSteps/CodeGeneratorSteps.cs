...
public class CodeGeneratorSteps
{
    ...
    private string _inputSchemaFileName;
    private Stream _inputStream;
    private bool _nullableGenEnable;
    private bool _nullableSupportEnable;
    ...
    public static Stream LoadResource(string name)
    {
        ...
        var urn = Array.Find(names, n => n.EndsWith(name, StringComparison.OrdinalIgnoreCase));
    ...
    public void ThenTheGeneratedOutputMustMatchTheReference()
    {
        ...
        var generated = _result.GeneratedFiles.Single().GeneratedContent;
        var equals = _referenceOutputContent.Equals(generated);
        if (!equals)
        {
            var path = Path.ChangeExtension(Path.GetTempFileName(), ".capnp.cs");
            File.WriteAllText(path, generated);

            var refLines = _referenceOutputContent.Split(Environment.NewLine);
            var actLines = generated.Split(Environment.NewLine);
            var mismatchLine = 0;

            for (var i = 0; i < Math.Min(refLines.Length, actLines.Length); i++)
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), _inputSchemaFileName);
        File.WriteAllText(path, _inputSchema);
        _result = CapnpCompilation.InvokeCapnpAndGenerate(new[] { path });
    }
