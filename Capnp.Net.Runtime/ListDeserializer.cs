...
public abstract class ListDeserializer
{
    ...
    ///     If this kind of list cannot be represented as list of lists (because it is a
    ///     list of non-pointers)
    /// </exception>
    ...
    /// <exception cref="NotSupportedException">
    ///     If this kind of list cannot be represented as list of capabilities (because it
    ///     is a list of non-pointers)
    /// </exception>
    ...
    /// <returns>
    ///     The desired representation as <![CDATA[IReadOnlyList<...IReadOnlyList<T>>]]></returns>
    ...
    public object CastND<T>(int n)
    {
        return CastND(n, ld => ld.Cast<IReadOnlyList<T>>());
    }
    ...
    /// <param name="n">Number of dimensions</param>
    /// <param name="cons">
    ///     Selector function which constructs an instance of <typeparamref name="T" /> from a
    ...
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public object CastND<T>(int n, Func<DeserializerState, T> cons)
    {
        return CastND(n, ld => ld.Cast(cons));
    }
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="n" /> is less than or equals 0.</exception>
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public object CastEnumsND<T>(int n, Func<ushort, T> cons)
    {
        return CastND(n, ld => ld.CastEnums(cons));
    }
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="n" /> is less than or equals 0</exception>
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public object CastVoidND(int n)
    {
        return CastND(n, ld => ld.Count);
    }
    ...
    /// <returns>The desired representation</returns>
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public IReadOnlyList<IReadOnlyList<byte>> CastData()
    {
        return Cast2D<byte>();
    }
    ...
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public IReadOnlyList<int> CastVoid2D()
    {
        return CastList().LazyListSelect(ld => ld.Count);
    }
    ...
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public IReadOnlyList<IReadOnlyList<int>> CastVoid3D()
    {
        return CastList().LazyListSelect(ld => ld.CastVoid2D());
    }
    ...
    /// <returns>The desired representation</returns>
    /// <exception cref="NotSupportedException">If this list cannot be represented in the desired manner.</exception>
    public IReadOnlyList<string?> CastText2()
    {
        return CastList().LazyListSelect(ld => ld.CastText());
    }
    ...
    ///     CastX actually means "represent this list as list of X". Logically, the semantics of CastText should be the
    ///     semantics
    ///     implemented by CastText2. And this method's name should be "CastChar". This wouldn't be accurate either, since a
    ///     string
    ///     is semantically more than the list of its characters. Trying to figure out a consistent naming pattern, we'd
    ///     probably
    ///     end up in less concise method names (do you have a good suggestion?). Considering this and the fact that you
    ///     probably
    ///     won't use these methods directly (because the code generator will produce nice wrappers for you) it seems
    ///     acceptable to
    ...
    private static class GenericCasts<T>
    {
