...
public static class Impatient
{
    ...
    internal static IRpcEndpoint? AskingEndpoint =>
        _askingEndpoint.Value!.Count > 0 ? _askingEndpoint.Value.Peek() : null;
    ...
    ///     <typeparamref name="TInterface" /> did not
    ...
    /// <param name="allowNoPipeliningFallback">
    ///     If this flag is 'false', the <paramref name="task" /> MUST have been returned from a remote
    ///     method invocation on a generated Proxy interface. Since this is the prerequisite for promise pipelining to work,
    ///     the method throws an
    ///     exception if the requirement is not met (i.e. the passed some Task instance was constructed "somewhere else").
    ///     Setting this flag to 'true'
    ///     prevents such an exception. The method falls back to a local "lazy" proxy for the given Task. It is fully usable,
    ///     but does not perform
    ///     any promise pipelining (as specified for Cap'n Proto).
    /// </param>
    ...
    ///     The task was not returned from a remote method invocation. Promise pipelining won't work.
    ///     Setting <paramref name="allowNoPipeliningFallback" />> to 'true' prevents this exception.
    ///     OR: Mismatch between generic type arguments (if capability interface is generic).
    /// </exception>
