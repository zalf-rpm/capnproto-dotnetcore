...
/// <summary>
///     Provides functionality to construct Proxy and Skeleton instances from capability interfaces and objects
///     implementing capability interfaces.
///     A capability interface is any .NET interface which is annotated with <see cref="ProxyAttribute" /> and
///     <see cref="SkeletonAttribute" />.
///     There are some intricacies to consider that you usually don't need to care about, since all that stuff will be
///     generated.
/// </summary>
public static class CapabilityReflection
{
    private static readonly ConditionalWeakTable<Type, ProxyFactory> _proxyMap = new();

    private static readonly ConditionalWeakTable<Type, SkeletonFactory> _skeletonMap = new();

    private static readonly ConditionalWeakTable<object, Skeleton> _implMap = new();
    ...
    private static SkeletonFactory CreateMonoSkeletonFactory(SkeletonAttribute attr, Type[] genericArguments)
    {
    ...
    private static SkeletonFactory GetSkeletonFactory(Type type)
    {
        return _skeletonMap.GetValue(type, _ =>
        {
            try
            {
                ...
                if (attrs.Length == 1) return CreateMonoSkeletonFactory(attrs[0].Attr, attrs[0].Generics);
    ...
    /// <exception cref="InvalidCapabilityInterfaceException">
    ///     No <see cref="SkeletonAttribute" /> found on implemented
    ///     interface(s).
    /// </exception>
    /// <exception cref="InvalidOperationException">
    ///     Mismatch between generic type arguments (if capability interface is
    ///     generic).
    /// </exception>
    ...
    ///     Problem with instatiating the Skeleton (constructor threw
    ///     exception).
    /// </exception>
    /// <exception cref="MemberAccessException">Caller does not have permission to invoke the Skeleton constructor.</exception>
    /// <exception cref="TypeLoadException">
    ///     Problem with building the Skeleton type, or problem with loading some dependent
    ///     class.
    /// </exception>
    [Obsolete(
        "Do not use this method directly. Instead, pass objects directly or use Proxy.Share<T>(). This method will be removed with next release.")]
    public static Skeleton CreateSkeleton(object obj)
    {
        return CreateSkeletonInternal(obj);
    }
    ...
    private static ProxyFactory GetProxyFactory(Type type)
    {
        return _proxyMap.GetValue(type, _ =>
        {
            try
            {
                ...
                throw new InvalidCapabilityInterfaceException(
                    "Multiple 'Proxy' attributes defined, don't know which one to take");
            }
            catch (System.Exception exception)
            {
    ...
    /// <summary>
    ///     Validates that a given type qualifies as cpapbility interface, throws
    ///     <see cref="InvalidCapabilityInterfaceException" /> on failure.
    /// </summary>
    ...
    /// <exception cref="InvalidCapabilityInterfaceException">
    ///     <typeparamref name="TInterface" /> did not qualify as capability
    ///     interface.
    /// </exception>
    /// <exception cref="InvalidOperationException">
    ///     Mismatch between generic type arguments (if capability interface is
    ...
    /// <exception cref="ArgumentException">Mismatch between generic type arguments (if capability interface is generic).</exception>
    /// <exception cref="System.Reflection.TargetInvocationException">
    ///     Problem with instatiating the Proxy (constructor threw
    ///     exception).
    /// </exception>
    ...
    private interface IBrokenFactory
    {
        System.Exception Exception { get; }
    }

    private abstract class ProxyFactory
    {
        public abstract Proxy NewProxy();
    }

    private class ProxyFactory<T> : ProxyFactory where T : Proxy, new()
    {
    ...
    private class BrokenProxyFactory : ProxyFactory, IBrokenFactory
    {
    ...
    private abstract class SkeletonFactory
    {
        public abstract Skeleton NewSkeleton();
    }

    private class SkeletonFactory<T> : SkeletonFactory where T : Skeleton, new()
    {
    ...
    private class BrokenSkeletonFactory : SkeletonFactory, IBrokenFactory
    {
    ...
    private class PolySkeletonFactory : SkeletonFactory
    {
        private readonly SkeletonFactory[] _monoFactories;
        ...
        public override Skeleton NewSkeleton()
        {
            ...
            foreach (var fac in _monoFactories) poly.AddInterface(fac.NewSkeleton());
            return poly;
        }
    }
}