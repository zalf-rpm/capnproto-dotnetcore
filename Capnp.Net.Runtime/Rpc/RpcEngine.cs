...
{
    private readonly ConcurrentBag<RpcEndpoint> _inboundEndpoints = new();

    private Skeleton? _bootstrapCap;
    ...
    public class RpcEndpoint : IEndpoint, IRpcEndpoint
    {
        ...
        private static readonly ThreadLocal<bool> _canDeferCalls = new();
        private static readonly ThreadLocal<FlushContext?> _flushRequests = new();
        private readonly Dictionary<uint, PendingAnswer> _answerTable = new();
        private readonly object _callReturnBlocker = new();
        private readonly Dictionary<uint, RefCounted<Skeleton>> _exportTable = new();

        private readonly RpcEngine _host;

        private readonly Dictionary<uint, RefCounted<RemoteCapability>> _importTable = new();
        private readonly Dictionary<uint, TaskCompletionSource<int>> _pendingDisembargos = new();
        private readonly Dictionary<uint, PendingQuestion> _questionTable = new();
        private readonly object _reentrancyBlocker = new();
        private readonly Dictionary<Skeleton, uint> _revExportTable = new();
        private readonly IEndpoint _tx;
        ...
        private ILogger Logger { get; } = Logging.CreateLogger<RpcEndpoint>();
        ...
        private uint AllocateExport(Skeleton providedCapability, out bool first)
        {
            lock (_reentrancyBlocker)
            {
        ...
        private void ProcessCall(Call.READER req)
        {
        ...
        private void ProcessCallLocked(Call.READER req)
        {
        ...
        private void ProcessReturn(Return.READER req)
        {
        ...
        private void ProcessResolve(Resolve.READER resolve)
        {
            lock (_reentrancyBlocker)
            {
        ...
        private void ProcessDisembargo(Disembargo.READER disembargo)
        {
        ...
        private void ReleaseExport(uint id, uint count)
        {
            bool exists;
        ...
        private void ProcessRelease(Release.READER release)
        {
        ...
        private void ProcessUnimplemented(Message.READER unimplemented)
        {
        ...
        private ConsumedCapability ImportCap(CapDescriptor.READER capDesc)
        {
            lock (_reentrancyBlocker)
            {
                switch (capDesc.which)
        ...
        {
            private readonly FlushContext? _prev;
        ...
        private readonly struct FlushContextKeeper : IDisposable
        {
            private readonly FlushContext _context;
