...
public static class MidlayerExtensions
{
    ...
    ///     Buffer size (bytes). You should choose it according to the maximum expected raw capnp frame
    ///     size
    /// </param>
    public static void AddBuffering(this ISupportsMidlayers obj, int bufferSize)
    {
        obj.InjectMidlayer(s => new DuplexBufferedStream(s, bufferSize));
    }
    ...
    {
        obj.InjectMidlayer(s => new DuplexBufferedStream(s));
    }
}