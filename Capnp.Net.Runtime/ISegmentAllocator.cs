...
public interface ISegmentAllocator
{
    ...
    ///     <paramref name="preferredSegment" />.
    ///     If that segment does not provide enough space or does not exist, further actions depend on the
    ///     <paramref name="forcePreferredSegment" /> flag.
    ///     If that flag is true, allocation will fail (return false). Otherwise, the allocation shall scan existing segments
    ///     for the requested amount of space,
