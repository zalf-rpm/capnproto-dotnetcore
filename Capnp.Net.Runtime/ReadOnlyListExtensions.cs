...
{
    /// <summary>
    ///     LINQ-like "Select" operator for <see cref="IReadOnlyList{T}" />, with the addition that the resulting elements are
    ///     accessible by index.
    ...
    ///     As opposed to <see cref="LazyListSelect{From, To}(IReadOnlyList{From}, Func{From, To})" /> the source is evaluated
    ///     immediately
    ...
    private class ReadOnlyListSelectOperator<From, To> : IReadOnlyList<To>
    {
        private readonly Func<From, To> _selector;
        private readonly IReadOnlyList<From> _source;
