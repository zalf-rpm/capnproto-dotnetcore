...
///     Although it is public, you should not use it directly. Instead, use the reader, writer, and domain class adapters
///     which are produced
///     by the code generator. Particularly, those writer classes are actually specializations of SerializerState, adding
///     convenience methods
...
public class SerializerState : IStructSerializer, IDisposable
{
    ...
    private SerializerState[]? _linkedStates;
    ...
    internal List<ConsumedCapability>? Caps => MsgBuilder?.Caps;
    ...
    ///     The data slice specified by <paramref name="bitOffset" /> and <paramref name="bitCount" />
    ///     is not completely within the struct's data section, misaligned, exceeds one word, or <paramref name="bitCount" />
    ///     is negative
    /// </exception>
    ...
    /// <exception cref="InvalidOperationException">The object was not determined to be a struct</exception>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     The data slice specified by <paramref name="bitOffset" /> and <paramref name="count" />
    ///     is not completely within the struct's data section, misaligned, exceeds one word, or <paramref name="count" /> is
    ///     negative
    /// </exception>
    ...
    /// <summary>
    ///     Represents this state by a different serializer state specialization. This is similar to a type-cast: The
    ...
    public TS Rewrap<TS>() where TS : SerializerState, new()
    {
        ...
        if (Kind != ObjectKind.Nil)
        {
            ...
            {
                return new InvalidOperationException("Incompatible cast");
            }
    ...
    private Span<ulong> FarSpan(uint index)
    {
        return Allocator!.Segments[(int)index].Span;
    }

    private void AllocateWords(uint count)
    {
        if (count == 0)
        {
            SegmentIndex = 0;
    ...
    /// <summary>
    ///     Allocates storage for the underlying object. Does nothing if it is already allocated. From the point the object is
    ///     allocated, its type cannot be changed
    ...
    /// <summary>
    ///     Links a sub-item (struct field or list element) of this state to another state. Usually, this operation is not
    ///     necessary, since objects are constructed top-down.
    ///     However, there might be some advanced scenarios where you want to reference the same object twice (also interesting
    ///     for designing amplification attacks).
    ...
    ///     Whether to deep copy the target state if it belongs to a different message builder than this
    ///     state.
    /// </param>
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>This state does neither describe a struct, nor a list of pointers</description>
    ///         </item>
    ///         <item>
    ///             <description>Another state is already linked to the specified position (sorry, no overwrite allowed)</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 This state and <paramref name="target" /> belong to different message builder, and
    ///                 <paramref name="allowCopy" /> is false
    ///             </description>
    ///         </item>
    ///     </list>
    /// </exception>
    ...
    private static InvalidOperationException AlreadySet()
    {
        return new InvalidOperationException("The object type was already set");
    }

    private static InvalidOperationException Unbound()
    {
        return new InvalidOperationException("This state is not bound to a MessageBuilder");
    }
    ...
    /// <exception cref="InvalidOperationException">The object type was already set to something different</exception>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     <paramref name="bitsPerElement" /> outside allowed range,
    ///     <paramref name="totalCount" /> negative or exceeding 2^29-1
    /// </exception>
    ...
    /// <exception cref="InvalidOperationException">The object type was already set to something different</exception>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     <paramref name="totalCount" /> negative, or total word count would exceed
    ///     2^29-1
    /// </exception>
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>The underlying object was not determined to be a struct or list of pointers.</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 Object at given position was already built and is not compatible with the desired target
    ///                 serializer type.
    ///             </description>
    ///         </item>
    ///     </list>
    ...
    ///         </item>
    ///         <item>
    ///             <description>Object at given position is not compatible with the desired target serializer type.</description>
    ///         </item>
    ///     </list>
    /// </exception>
    /// <exception cref="IndexOutOfRangeException"><paramref name="index" /> is out of bounds.</exception>
    ...
    /// </exception>
    /// <exception cref="IndexOutOfRangeException"><paramref name="index" /> is out of bounds.</exception>
    public DynamicSerializerState BuildPointer(int index)
    {
        return BuildPointer<DynamicSerializerState>(index);
    }
    ...
    /// </exception>
    /// <exception cref="IndexOutOfRangeException"><paramref name="index" /> is out of bounds.</exception>
    public SerializerState? TryGetPointer(int index)
    {
        return TryGetPointer<SerializerState>(index);
    }
    ...
    ///         </item>
    ///         <item>
    ///             <description>Object at given position was already set.</description>
    ///         </item>
    ///     </list>
    /// </exception>
    /// <exception cref="IndexOutOfRangeException"><paramref name="index" /> is out of bounds.</exception>
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    ///     The given element count does not match the underlying list's element
    ///     count.
    /// </exception>
    ...
    /// <exception cref="InvalidOperationException">
    ///     The underlying message builder was not configured for capability table
    ///     support.
    /// </exception>
    ...
    ///     support.
    /// </exception>
    public uint? ProvideCapability(Skeleton capability)
    {
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>Low-level capability (<see cref="Rpc.ConsumedCapability" />)</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 Proxy object (<see cref="Rpc.Proxy" />). Note that the provision has "move semantics": SerializerState
    ///                 takes ownership, so the Proxy object will be disposed.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description><see cref="Rpc.Skeleton" /> instance</description>
    ///         </item>
    ///         <item>
    ///             <description>Capability interface implementation</description>
    ///         </item>
    ///     </list>
    /// </param>
    /// <returns>Index of the given capability in the capability table</returns>
    ...
    public uint? ProvideCapability(object? obj)
    {
        switch (obj)
        {
            ...
            case ConsumedCapability consumedCapability:
                return ProvideCapability(consumedCapability);
            case Skeleton providedCapability:
                return ProvideCapability(providedCapability);
            default:
                return ProvideCapability(CapabilityReflection.CreateSkeletonInternal(obj));
        }
    }

    /// <summary>
    ///     Links a sub-item (struct field or list element) of this state to another object.
    ///     In contrast to <see cref="Link(int, SerializerState, bool)" />, this method also accepts deserializer states,
    ///     domain objects, capabilites, and lists thereof.
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>Another <see cref="SerializerState" /></description>
    ///         </item>
    ///         <item>
    ///             <description>A <see cref="DeserializerState" /> (will always deep copy)</description>
    ///         </item>
    ///         <item>
    ///             <description>An object implementing <see cref="ICapnpSerializable" /></description>
    ///         </item>
    ///         <item>
    ///             <description>A low-level capability object (<see cref="Rpc.ConsumedCapability" />)</description>
    ///         </item>
    ///         <item>
    ///             <description>A proxy object (<see cref="Rpc.Proxy" />)</description>
    ///         </item>
    ///         <item>
    ///             <description>A skeleton object (<see cref="Rpc.Skeleton" />)</description>
    ///         </item>
    ///         <item>
    ///             <description>A capability interface implementation</description>
    ///         </item>
    ///         <item>
    ///             <description>A <see cref="IReadOnlyList{T}" /> of one of the things listed here.</description>
    ///         </item>
    ///     </list>
    /// </param>
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>Object neither describes a struct, nor a list of pointers, nor a capability</description>
    ///         </item>
    ///         <item>
    ///             <description>Another state is already linked to the specified position (sorry, no overwrite allowed)</description>
    ///         </item>
