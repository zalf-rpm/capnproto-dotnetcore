...
/// <summary>
///     Implements the heart of deserialization. This stateful helper struct exposes all functionality to traverse
///     serialized data.
...
public struct DeserializerState : IStructDeserializer, IDisposable
{
    ...
    /// <summary>
    ///     Context-dependent meaning: Usually the number of bytes traversed until this state was reached, to prevent
    ///     amplification attacks.
    ///     However, if this state is of Kind == ObjectKind.Value (an artificial category which will never occur on the wire
    ///     but is used to
    ...
    /// </summary>
    public IList<ConsumedCapability>? Caps { get; set; }
    ...
    {
        DeserializerState state = new(frame.Segments);
    ...
    /// <summary>
    ///     Constructs a state representing the given value. This kind of state is artificial and beyond the Cap'n Proto
    ///     specification.
    ...
    private ReadOnlySpan<ulong> GetRawBits()
    {
        return CurrentSegment.Slice(Offset, (ListElementCount + 63) / 64);
    }

    private ReadOnlySpan<ulong> GetRawBytes()
    {
        return CurrentSegment.Slice(Offset, (ListElementCount + 7) / 8);
    }

    private ReadOnlySpan<ulong> GetRawShorts()
    {
        return CurrentSegment.Slice(Offset, (ListElementCount + 3) / 4);
    }

    private ReadOnlySpan<ulong> GetRawInts()
    {
        return CurrentSegment.Slice(Offset, (ListElementCount + 1) / 2);
    }

    private ReadOnlySpan<ulong> GetRawLongs()
    {
        return CurrentSegment.Slice(Offset, ListElementCount);
    }
        ...
        do
        {
            ...
            switch (pointer.Kind)
            {
                ...
                case PointerKind.Other:
                    IList<ConsumedCapability>? tmp = Caps;
                    this = default;
                    Caps = tmp;
                    Kind = ObjectKind.Capability;
    ...
    /// <summary>
    ///     Interprets a pointer within the current segment as capability pointer and returns the according low-level
    ///     capability object from
    ...
    public ulong StructReadData(ulong bitOffset, int bitCount)
    {
        switch (Kind)
        {
            ...
            case ObjectKind.Struct:
                ...
                ReadOnlySpan<ulong> data = StructDataSection;
    ...
    ///     It is valid to specify an index beyond the pointer section, in which case a default state (representing the "null
    ///     object")
    ...
    /// <summary>
    ///     Given this state represents a list of pointers, returns a ListOfCapsDeserializer for decoding it as list of
    ///     capabilities.
    /// </summary>
    ...
    public bool IsStructFieldNonNull(int index)
    {
        ...
        int pointerOffset = index + StructDataCount;
    ...
    public T? ReadCap<T>(int index)
        where T : class
    {
        ConsumedCapability cap = StructReadRawCap(index);
    ...
    ///     non-capability pointer, traversal limit exceeded
    /// </exception>
    public BareProxy ReadCap(int index)
    {
        ConsumedCapability cap = StructReadRawCap(index);
        ...
        where T : class
    {
        ...
        return (CapabilityReflection.CreateProxy<T>(Caps[(int)CapabilityIndex]) as T)!;
    }
